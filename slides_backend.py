...
def generate_text_formatting_requests(object_id, text_elements, cell_location=None):
    ...
    for text_element in text_elements:
        if 'paragraphMarker' in text_element:
            ...
            # Skip if the paragraph has no length
            ...
            # --- Handle Paragraph Styles (Alignment, Spacing, Indents, etc.) ---
            paragraph_style = master_paragraph_marker.get('style', {})
            styles_to_apply = {
                k: paragraph_style.get(k) for k in
                ['alignment', 'lineSpacing', 'spaceAbove', 'spaceBelow', 'direction', 'indentStart', 'indentFirstLine']
                if k in paragraph_style
            }

            # --- Handle Bullet Logic ---
            if 'bullet' in master_paragraph_marker:
                # Add the entire bullet object to the style. This creates/updates the bullet.
                styles_to_apply['bullet'] = master_paragraph_marker['bullet']
            else:
                # If master has NO bullet, explicitly remove any existing bullet from the destination.
                remove_req = {'removeParagraphBullets': {'objectId': object_id, 'textRange': text_range}}
                if cell_location: remove_req['removeParagraphBullets']['cellLocation'] = cell_location
                requests.append(remove_req)

            # --- Create the final UpdateParagraphStyle request if styles were found ---
            if styles_to_apply:
                # Use the recursive helper to create a mask that handles nested objects like 'bullet'.
                fields_mask = ",".join(_create_recursive_field_mask(styles_to_apply))
