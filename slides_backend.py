                ...
                try: # --- DEFENSIVE SLIDE ITERATION TRY/EXCEPT ---
                    ...
                    if structural_reqs:
                        ...
                        try:
                            ...
                            # Iterate through original requests and replies in parallel
                            for i, req in enumerate(structural_reqs):
                                action_key = next(iter(req))
                                # We only care about mapping IDs for 'create' requests
                                if 'create' in action_key:
                                    master_id = req[action_key].get('objectId')

                                    # The reply for a create request contains the new ID.
                                    # The order of replies matches the order of requests.
                                    reply = replies[i]
                                    new_id = next((v for k, v in reply.items() if 'objectId' in v), {}).get('objectId')

                                    if master_id and new_id:
                                        id_map[master_id] = new_id

                            logging.debug(f"ID Map correctly built with {len(id_map)} entries.")
