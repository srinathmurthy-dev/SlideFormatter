...
def copy_slide_content(slides_service, master_slide_id, master_pres_id, dest_pres_id, dest_slide_id, master_slide_json, drive_service, docai_client, storage_client):
    ...
    for master_element in master_elements:
        ...
        if element_type == 'image':
            ocr_elements = process_image_ocr(master_element, drive_service, docai_client, storage_client, slides_service, master_pres_id, master_slide_id)
            if ocr_elements:
                keyword_metadata = process_text_bearing_objects(slides_service, dest_pres_id, dest_slide_id, ocr_elements, "Format 1: Row 0/Col 0 Headers")
                GLOBAL_METADATA['Keyword_Values'].extend(keyword_metadata)

        # 2B: Apply Basic Object Styling
        if element_type == 'shape':
            shape_props = master_element.get('shape', {}).get('shapeProperties', {})
            styles_to_apply = {}
            if 'shapeBackgroundFill' in shape_props: styles_to_apply['shapeBackgroundFill'] = shape_props.get('shapeBackgroundFill')
            if 'outline' in shape_props: styles_to_apply['outline'] = shape_props.get('outline')
            
            if styles_to_apply:
                formatting_requests.append({
                    'updateShapeProperties': {
                        'objectId': element_id,
                        'shapeProperties': styles_to_apply,
                        'fields': ",".join(_create_recursive_field_mask(styles_to_apply))
                    }
                })

        # For tables, we'll handle borders in a separate, more specific request if needed.
        # Currently, the user has not requested table-specific styling beyond text.

        # 2C: Apply Text Content and Styling
        if master_element.get(element_type, {}).get('text'):
            text_elements = master_element[element_type]['text']['textElements']
            
            if element_type == 'table':
                # For tables, the helper function handles cell-by-cell text and style updates.
                formatting_requests.extend(
                    process_table_for_replication(element_id, dest_pres_id, dest_slide_id, master_element)
                )
            elif element_type == 'shape':
                # For shapes, first delete and insert the raw text.
                full_text = get_text_content_from_element(master_element)
                if full_text.strip():
                    formatting_requests.append({'deleteText': {'objectId': element_id, 'textRange': {'type': 'ALL'}}})
                    formatting_requests.append({'insertText': {'objectId': element_id, 'text': full_text}})

                # Then, apply the specific styles for each text run.
                formatting_requests.extend(
                    generate_text_style_requests(element_id, text_elements)
                )
        ...
        for dest_file in destination_files:
            ...
            for i, master_slide in enumerate(master_slide_list):
                ...
                try: # --- DEFENSIVE SLIDE ITERATION TRY/EXCEPT ---
                    ...
                    # Run sync logic to get structural and formatting requests.
                    ...
                    if structural_reqs:
                        ...
                        try:
                            ...
                            logging.info(f"Applied {len(structural_reqs)} structural batch updates successfully.")

                            # --- Build the ID Map Correctly ---
                            replies = response.get('replies', [])
                            create_requests = [(i, req) for i, req in enumerate(structural_reqs) if 'create' in next(iter(req))]

                            logging.debug(f"ID_MAP: Found {len(create_requests)} create requests to map from {len(replies)} total replies.")
                            ...
                            for i, req in create_requests:
                                if i < len(replies):
                                    ...
                                    if action_key in reply:
                                        new_id = reply[action_key].get('objectId')
                                        if master_id_val and new_id:
                                            id_map[master_id_val] = new_id
                                            logging.debug(f"ID_MAP_ENTRY: Mapped master ID '{master_id_val}' -> new dest ID '{new_id}'")
                                        else:
                                            logging.warning(f"ID_MAP_FAIL: Missing master_id ('{master_id_val}') or new_id ('{new_id}') in request/reply pair.")
                                    else:
                                        logging.warning(f"ID_MAP_FAIL: Reply at index {i} did not contain expected key '{action_key}'. Reply: {reply}")
                                else:
                                    logging.error(f"ID_MAP_ERROR: Request index {i} is out of bounds for replies list (len {len(replies)}).")

                            logging.info(f"ID Map built with {len(id_map)} entries.")

                        except HttpError as e:
                            logging.error(f"STRUCTURAL BatchUpdate failed on {dest_name}, slide {i+1}: {e}")
                            total_inconsistencies += 1
                            continue # If structure fails, skip formatting for this slide

                    # --- Remap and Execute Formatting Changes Second ---
                    if formatting_reqs:
                        # Before executing, remap the object IDs in the formatting requests.
                        for req in formatting_reqs:
                            action_key = next(iter(req))
                            if 'objectId' in req[action_key]:
                                master_obj_id = req[action_key]['objectId']
                                if master_obj_id in id_map:
                                    logging.debug(f"ID_REMAP: Remapping master object {master_obj_id} to new destination ID {id_map[master_obj_id]}.")
                                    req[action_key]['objectId'] = id_map[master_obj_id]
                                else:
                                    # This is not necessarily an error; the object might have existed before and not needed remapping.
                                    logging.debug(f"ID_REMAP_SKIP: Master object ID {master_obj_id} not in created ID map; assuming it's a pre-existing object.")
