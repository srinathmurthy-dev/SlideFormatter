...
def _create_recursive_field_mask(properties, parent_key=''):
    ...
    This is the correct way to generate a field mask for nested objects in the Slides API.
    For example: "shapeProperties.outline.outlineFill.solidFill.color"
    """
    mask_paths = []
    for key, value in properties.items():
        # Construct the current path for this key
        ...
        # If it's a non-dictionary value, we've reached a leaf node. Add the path.
        else:
    ...
    for master_element in master_elements:
        ...
        if matching_dest_id:
            ...
            if element_type == 'shape' and 'shapeProperties' in master_element['shape']:
                ...
                writable_props = _scrub_read_only_fields(master_element['shape']['shapeProperties'])

                update_request = {
                    'updateShapeProperties': {
                        'objectId': new_element_id,
                        'shapeProperties': writable_props,
                        'fields': ",".join(_create_recursive_field_mask(writable_props))
                    }
                }
