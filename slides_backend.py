...
def copy_slide_content(slides_service, master_slide_id, master_pres_id, dest_pres_id, dest_slide_id, master_slide_json, drive_service, docai_client, storage_client):
    """
    Performs synchronization in two phases, returning requests for each phase and
    a list of master IDs for newly created objects to facilitate ID mapping.
    """
    structural_requests = []
    formatting_requests = []
    created_master_ids = []
    ...
    for master_element in master_elements:
        if not find_master_match(master_element, dest_elements):
            ...
            if create_request:
                ...
                created_master_ids.append(new_element_id)
    ...
    return structural_requests, formatting_requests, created_master_ids
        ...
        for dest_file in destination_files:
            ...
            for i, master_slide in enumerate(master_slide_list):
                ...
                try: # --- DEFENSIVE SLIDE ITERATION TRY/EXCEPT ---
                    ...
                    # Run sync logic to get structural/formatting requests and a list of master IDs for created objects.
                    structural_reqs, formatting_reqs, created_master_ids = copy_slide_content(
                        slides_service, master_slide_id, master_id, dest_id, dest_slide_id,
                        master_slide, drive_service, docai_client, storage_client
                    )
                    
                    # ID Map for re-targeting formatting requests to newly created objects.
                    id_map = {}
                    ...
                    if structural_reqs:
                        ...
                        try:
                            ...
                            # Build the ID map from the response
                            if created_master_ids:
                                replies = response.get('replies', [])
                                for i, master_id in enumerate(created_master_ids):
                                    # Find the corresponding 'create' reply and get the new ID.
                                    # The replies are in the same order as the requests.
                                    create_reply = replies[i]
                                    new_id = next((v for k, v in create_reply.items() if 'objectId' in v), {}).get('objectId')
                                    if new_id:
                                        id_map[master_id] = new_id
                                logging.debug(f"ID Map created with {len(id_map)} entries.")
                    ...
                    if formatting_reqs:
                        ...
                        for req in formatting_reqs:
                            # The objectId is always at the top level of the request body.
                            # e.g., {'updateShapeProperties': {'objectId': 'MASTER_ID', ...}}
                            action_key = next(iter(req))
                            if 'objectId' in req[action_key]:
                                master_id = req[action_key]['objectId']
                                if master_id in id_map:
                                    req[action_key]['objectId'] = id_map[master_id]
