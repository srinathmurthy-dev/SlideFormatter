...
def copy_slide_content(slides_service, master_slide_id, master_pres_id, dest_pres_id, dest_slide_id, master_slide_json, drive_service, docai_client, storage_client):
    ...
    for master_element in master_elements:
        ...
        # 2C: Apply Text Content - Differentiated by element type
        if element_type == 'table':
            # For tables, use the helper that iterates through cells
            formatting_requests.extend(
                process_table_for_replication(element_id, dest_pres_id, dest_slide_id, master_element)
            )
        elif element_type == 'shape':
            # For shapes, apply text directly
            ...
            if full_text.strip():
                ...
                # Insert new text
                formatting_requests.append({'insertText': {'objectId': element_id, 'text': full_text.strip()}})
...
def process_table_for_replication(table_id, dest_pres_id, dest_slide_id, table_element):
    """Generates API requests to replicate the text content of a master table."""
    requests = []

    # This function is now only called for tables, so we can safely access 'table'
    table_prop = table_element['table']
    rows = table_prop['rows']
    cols = table_prop['columns']

    logging.debug(f"Processing Table {table_id} text content.")
    
    for r_idx in range(rows):
        for c_idx in range(cols):
            try:
                cell = table_prop['tableRows'][r_idx]['tableCells'][c_idx]
                cell_content = get_text_content_from_element(cell).strip()
                
                if cell_content: 
                    # Delete existing text to ensure a clean slate
                    requests.append({
                        'deleteText': {
                            'objectId': table_id, 
                    ...
                    # Insert the new text from the master
                    requests.append({
                        'insertText': {
                            'objectId': table_id, 
                            'cellLocation': {'rowIndex': r_idx, 'columnIndex': c_idx}, 
                ...
                logging.warning(f"Error parsing table cell {r_idx},{c_idx} for text replication: {e}")

    logging.debug(f"Generated {len(requests)} text requests for table {table_id}.")
    return requests
